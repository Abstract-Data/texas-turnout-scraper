...
def _make_roster(
    county: str, records: list[VoterRecord], report_date: date = date(2026, 2, 27)
) -> CountyRoster:
    return CountyRoster(
        county=county,
        county_id=1,
...
def test_no_duplicates():
    ...
    report = audit_roster(
        rosters, election_id="53813", report_date=date(2026, 2, 27), source="civix"
    )
...
def test_missing_county_finding():
    ...
    missing = [f for f in report.findings if f.finding_type == FindingType.MISSING_COUNTY]
